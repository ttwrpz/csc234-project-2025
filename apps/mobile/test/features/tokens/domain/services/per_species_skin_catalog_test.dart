...
void main() {
  group('PerSpeciesSkinCatalog invariants', () {
    test('every species has exactly two alternate skins', () {
      for (final species in FlowerSpecies.values) {
        expect(
          PerSpeciesSkinCatalog.forSpecies(species).length,
          equals(2),
          reason: '$species should offer exactly two per-species skins',
        );
      }
    });

    test('catalog has 12 skins total (6 species x 2)', () {
      expect(PerSpeciesSkinCatalog.all.length, equals(12));
    });
