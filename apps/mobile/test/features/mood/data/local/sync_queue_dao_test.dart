...
void main() {
  ...
  group('Coalescing rules (ADR-0004)', () {
    test(
      'create then update for same entry_id → ONE row, latest payload',
      () async {
        await queue.enqueue(
    ...
    test(
      'update then update for same entry_id → ONE row, latest payload',
      () async {
        await queue.enqueue(
    ...
    test('create then delete for same entry_id → ONE row, op=delete', () async {
      await queue.enqueue(row(entryId: 'm1', operation: SyncOperation.create));
      await queue.enqueue(row(entryId: 'm1', operation: SyncOperation.delete));
      expect(await queue.length(), 1);
      final pending = await queue.peekNextDue(now: 9999);
      expect(pending!.operation, SyncOperation.delete);
    });

    test('update then delete for same entry_id → ONE row, op=delete', () async {
      await queue.enqueue(row(entryId: 'm1', operation: SyncOperation.update));
      await queue.enqueue(row(entryId: 'm1', operation: SyncOperation.delete));
      expect(await queue.length(), 1);
      final pending = await queue.peekNextDue(now: 9999);
      expect(pending!.operation, SyncOperation.delete);
    ...
    test('different entry_ids do NOT coalesce', () async {
      await queue.enqueue(row(entryId: 'm1', operation: SyncOperation.create));
      await queue.enqueue(row(entryId: 'm2', operation: SyncOperation.create));
  ...
  group('peekNextDue ordering and retry filtering', () {
    ...
    test('returns lowest-id row when multiple are due (FIFO)', () async {
      ...
      await queue.enqueue(row(entryId: 'a', operation: SyncOperation.create));
      await queue.enqueue(row(entryId: 'b', operation: SyncOperation.create));
  ...
  group('watchPendingCount stream', () {
    test(
      'emits 0 for empty queue, increments on enqueue, decrements on dequeue',
      () async {
