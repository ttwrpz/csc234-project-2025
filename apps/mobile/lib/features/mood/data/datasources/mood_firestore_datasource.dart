...
class MoodFirestoreDatasource {
  ...
  Future<MoodEntryDto> create(MoodEntryDto dto) async {
    // When the caller supplied an id (offline-first queue replay path), reuse
    // it as the Firestore doc id so that the post-write listener emission's
    // id matches the local Drift row's id and upsertFromRemote's LWW updates
    // the existing row instead of inserting an orphan duplicate. The
    // cloud-only fallback path (where id is empty) keeps the `.add()`
    // behavior so Firestore generates a fresh id.
